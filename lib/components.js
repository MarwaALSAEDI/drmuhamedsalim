...
function hero(c) {
  ...
  <div class="wrap relative">
    <!--
      الموبايل: عمودان — العنوان مع الصورة جنبه في أول شاشة، ثم بقية المحتوى بعرض كامل.
      الشاشات الكبيرة: عمود للنص وعمود للصورة يمتد على الصفّين.
    -->
    <div class="grid grid-cols-[minmax(0,1fr)_38%] items-start gap-x-5 gap-y-8 py-12
                sm:gap-x-8 lg:grid-cols-[minmax(0,1.2fr)_minmax(0,.9fr)] lg:items-center lg:gap-x-16 lg:py-20 xl:py-24">

      <!-- العنوان -->
      <div class="min-w-0 animate-fade-up lg:col-start-1 lg:row-start-1">
        <span class="eyebrow eyebrow-dark">${esc(h.eyebrow || s.tagline)}</span>
        <h1 class="h-hero mt-4 text-white lg:mt-5">${esc(h.h1)}</h1>
      </div>

      <!-- الصورة: بجانب العنوان على الموبايل، وعمود مستقل على الكبيرة -->
      <div class="relative animate-fade-up lg:col-start-2 lg:row-start-1 lg:row-span-2 lg:self-center lg:justify-self-end"
           style="animation-delay:.1s">
        <div class="relative mx-auto w-full lg:max-w-[26rem]">
          <div class="relative aspect-[4/5] overflow-hidden rounded-xl border border-white/10 bg-navy-900 lg:rounded-2xl">
            ${portrait}
            <div class="pointer-events-none absolute inset-x-0 bottom-0 h-1/3 bg-gradient-to-t from-navy-950/80 to-transparent"></div>
          </div>
          ${(h.chips || []).length ? `<div class="absolute inset-x-4 bottom-4 hidden flex-wrap gap-2 lg:flex">
      ...
      <!-- بقية المحتوى: بعرض العمودين على الموبايل -->
      <div class="col-span-2 animate-fade-up lg:col-span-1 lg:col-start-1 lg:row-start-2" style="animation-delay:.16s">
        <p class="lead-dark max-w-xl">${esc(h.lead)}</p>

        ${(h.chips || []).length ? `<div class="mt-6 flex flex-wrap gap-2 lg:hidden">
          ${(h.chips || []).map(x => `<span class="rounded-md border border-white/15 bg-white/[.06] px-3 py-1.5 text-[.82rem] font-medium text-navy-100">${esc(x)}</span>`).join('\n          ')}
        </div>` : ''}

        <div class="mt-7 lg:mt-9">${ctaGroup(c, { dark: true })}</div>

        <!-- معلومات أساسية: عمودان لتفادي انكسار قائمة الأيام -->
        <div class="mt-9 grid max-w-xl gap-x-8 gap-y-6 border-t border-white/10 pt-7 sm:grid-cols-2 lg:mt-10 lg:pt-8">
...
/* قسم التعريف — بلا صورة، لأن صورة الطبيب تظهر في الهيرو أعلاه */
function doctorIntro(c) {
  const home = c.home;

  return `<section class="section bg-soft">
  <div class="wrap">
    <div class="grid gap-10 lg:grid-cols-[minmax(0,.75fr)_minmax(0,1.25fr)] lg:gap-16">

      <!-- العنوان -->
      <div class="lg:sticky lg:top-28 lg:self-start">
        <span class="eyebrow reveal">عن الطبيب</span>
        <h2 class="h-section reveal mt-4">${esc(home.introTitle || 'تعرف على الطبيب')}</h2>
        <span class="reveal mt-6 hidden h-11 w-11 place-items-center rounded-lg border border-line bg-white text-brand-600 lg:grid">
          <span class="block h-5 w-5">${ICON.stethos}</span>
        </span>
      </div>

      <!-- المحتوى -->
      <div>
        <div class="prose-ar reveal text-[1.06rem]">${paras(c.about.bio)}</div>
        ...
        <div class="reveal mt-9 flex flex-wrap items-center gap-x-7 gap-y-4">
